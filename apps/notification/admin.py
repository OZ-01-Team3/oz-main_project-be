...
from apps.notification.models import GlobalNotification, GlobalNotificationConfirm, RentalNotification
...
class GlobalNotificationAdmin(admin.ModelAdmin):
    ...
    class GlobalNotificationConfirmInline(admin.StackedInline):
    ...
    def image_url(self, obj: GlobalNotification) -> str:
        if obj.image:
            return obj.image.url

    def send_notification_to_all_users(self, request: HttpRequest, queryset: QuerySet) -> None:
        notifications = list(queryset)
        users = Account.objects.all()
        confirm_objects = []
        for notification in notifications:
            confirm_objects.extend([
                GlobalNotificationConfirm(user=user, notification=notification) for user in users
            ])
    ...
    send_notification_to_all_users.short_description = "모든 유저에게 선택한 알림 전송하기"


@admin.register(GlobalNotificationConfirm)
class GlobalNotificationConfirmAdmin(admin.ModelAdmin):
    fieldsets = (
    ...
    def confirm_notification(self, request: HttpRequest, queryset: QuerySet) -> None:
        if queryset.exists():
    ...
    confirm_notification.short_description = "선택한 객체의 알림 확인상태로 변경"
...
class RentalNotificationAdmin(admin.ModelAdmin):
    ...
    def product_name(self, obj: RentalNotification) -> str:
        product_name = obj.rental_history.product.name
        return product_name

    def notification_confirm(self, request: HttpRequest, queryset: QuerySet) -> None:
        if queryset.exists():
            notifications = list(queryset)
    ...
    notification_confirm.short_description = "선택한 알림 확인 상태로 변경하기"
