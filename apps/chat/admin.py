...
class MessageAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("MessageInfo", {"fields": ("chatroom", "text", "image",)}),
    ...
    def set_status_false(self, request: HttpRequest, queryset: QuerySet) -> None:
        if queryset.exists():
            #쿼리셋의 객체들을 리스트화
    ...
    def set_status_true(self, request: HttpRequest, queryset: QuerySet) -> None:
        if queryset.exists():
            #쿼리셋의 객체들을 리스트화
            messages = list(queryset)
            # 각 객체의 읽음 여부를 업데이트.
    ...
    set_status_false.short_description = "선택한 메시지를 읽음으로 처리"
    set_status_true.short_description = "선택한 메시지를 안읽음으로 처리"
...
class ChatroomAdmin(admin.ModelAdmin):
    ...
    def leave_borrower(self, request: HttpRequest, queryset: QuerySet) -> None:
        if queryset.exists():
            ...
            # 남아 있는 객체들을 업데이트
            if chatrooms:
                Message.objects.bulk_update(chatrooms, ['borrower_status'])

    def leave_lender(self, request: HttpRequest, queryset: QuerySet) -> None:
        if queryset.exists():
            chatrooms = list(queryset)
            to_delete = []
            ...
            if chatrooms:
                Message.objects.bulk_update(chatrooms, ['lender_status'])

    leave_lender.short_description = '선택한 채팅방에서 판매자가 나가기'
    leave_borrower.short_description = '선택한 채팅방에서 대여자가 나가기'
